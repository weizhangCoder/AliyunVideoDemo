...
//        self.urlSource = [[AVPUrlSource alloc]urlWithString:@"https://static.mingchu.co/barrage.mp4"];
...
- (void)onSubtitleExtAdded:(AliPlayer *)player trackIndex:(int)trackIndex URL:(NSString *)URL{
    NSLog(@"%@",URL);
    [self.aliPlayer selectExtSubtitle:trackIndex enable:YES];
}
/**
 @brief 字幕显示回调
 @param player 播放器player指针
...
//                    }
        ...
        [self.loadingView show];
   //要是传入外挂字幕
        if (self.extSubtitleUrlStr.length > 0) {
            [self.aliPlayer addExtSubtitle:self.extSubtitleUrlStr];
        }
        [self.aliPlayer prepare];
        if (self.saveCurrentTime > 0) {
            [self seekTo:self.saveCurrentTime*1000];
