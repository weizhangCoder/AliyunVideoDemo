...
- (void)copyFromPlayerConfig{
    ...
    switch (self.sourceSelectView.selectIndex) {
        ...
        case 3: {
            ...
            break;
        case 4: {
            self.templiveStsSource = [[AVPLiveStsSource alloc] initWithUrl:self.playerConfig.liveStsSource.url accessKeyId:self.playerConfig.liveStsSource.accessKeyId accessKeySecret:self.playerConfig.liveStsSource.accessKeySecret securityToken:self.playerConfig.liveStsSource.securityToken region:self.playerConfig.liveStsSource.region domain:self.playerConfig.liveStsSource.domain app:self.playerConfig.liveStsSource.app stream:self.playerConfig.liveStsSource.stream encryptionType:self.playerConfig.liveStsSource.encryptionType];
        }
        break;
        default:
            break;
...
- (NSArray *)sourceKeyArray {
    switch (self.sourceSelectView.selectIndex) {
        ...
        case 4:
            return @[@"url",@"accessKeyId",@"accessKeySecret",@"securityToken",@"region",@"domain",@"app",@"stream",@"encryptionType"];
        default:
            return nil;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([cell.leaderText isEqualToString:@"encryptionType"]){
       cell.showSegment = YES;
       cell.showScanButton = NO;
        cell.segment.selectedSegmentIndex = self.playerConfig.liveStsSource.encryptionType;
        cell.sengmentChangedCallBack = ^(NSString *leaderText, NSInteger idx) {
